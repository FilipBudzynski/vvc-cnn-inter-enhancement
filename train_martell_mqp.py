"""MULTI-QP VARIANT (mqp): identical to train_martell_hybrid.py but
reads samples on the fly from VTM-encoded multi-QP data (VTMOnlineDataset
over output_vtm_train/{variant}) and uses 50 epochs over 4x data with
MultiStepLR milestones [13, 25, 38, 50] (the same number of optimisation
steps and the same schedule in steps as the 200-epoch baseline).
"""
...
def main():
    ...
    if args.ckpt_prefix is None:
        args.ckpt_prefix = f"martell_hybrid_mqp_{args.variant}"
    man = json.load(open(_P("output_vtm_train") / args.variant / "manifest_all.json"))
    train_ds = VTMOnlineDataset(man["tasks"], patch_size=args.patch_size, split="train")
    val_ds = VTMOnlineDataset(man["tasks"], patch_size=args.patch_size, split="val")
    ...
    sched = optim.lr_scheduler.MultiStepLR(optim_, milestones=[13, 25, 38, 50])
    ...
    for epoch in range(args.epochs):
        ...
        # Track best by the sum of per-channel gains (chroma rewarded)
