#!/usr/bin/env python3
"""
Compare: Old (epoch 360, L1+SSIM) vs New (epoch 140, L1+MS-SSIM+Gradient)
"""
...
print('Loading new model (epoch 140, L1+MS-SSIM+Gradient)...')
...
print(f'New Model (epoch 140, L1+MS-SSIM+Grad): {new_gain:+.4f} dB')
...
for batch in loader:
    ...
    axes[sample_idx, 4].set_title('Gradient Mag\n(Detail level)', fontsize=10)
    axes[sample_idx, 4].set_ylabel('Magnitude')
    
    sample_idx += 1

plt.suptitle('Comparison: Old (L1+SSIM) vs New (L1+MS-SSIM+Gradient Loss)', fontsize=16)
plt.tight_layout()
plt.savefig('model_comparison_epoch140.png', dpi=150, bbox_inches='tight')
print('\nSaved: model_comparison_epoch140.png')
