#!/usr/bin/env python3
"""
Visualize Snow-Wide model results - generate side-by-side comparisons
"""
...
model = SnowWideEnhancer(SnowWideConfig()).to(DEVICE)
model.load_state_dict(torch.load("checkpoints/snow_wide_epoch_360.pt", map_location=DEVICE))
model.eval()
...
for i, s in enumerate(sample_images):
    ...
    axes[i, 2].set_title(f"Snow-Wide Enhanced\nPSNR: {s['psnr_enh']:.2f}", fontsize=10)
    axes[i, 2].axis('off')
...
plt.tight_layout()
plt.savefig('snow_wide_visualization.png', dpi=150, bbox_inches='tight')
print(f"Saved: snow_wide_visualization.png")

print(f"\n=== Snow-Wide Test Results ({num_samples} samples) ===")
