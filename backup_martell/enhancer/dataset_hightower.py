...
class HightowerDataset(Dataset):
    """Dataset that loads neighboring frames for temporal enhancement"""
    ...
    def __init__(
        ...
        # Flatten and create valid indices (frame index 1 to n-2 for F-1, F0, F+1)
        self.samples = []
    ...
    def __getitem__(self, idx):
        ...
        # Extract YUV frames
        prev_frame = prev_data["decoded"]   # [3, H, W]
        curr_frame = curr_data["decoded"]
        next_frame = next_data["decoded"]
        original = curr_data["original"]    # Ground truth for current frame
        
        # Features (motion vectors + metadata) - use current frame's features
        features = curr_data["features"]   # [8, H, W]
        
        # Split features: first 4 for motion vectors, rest for metadata
        motion_vectors = features[:4]      # [4, H, W] - MVL0_X, MVL0_Y, MVL1_X, MVL1_Y
        metadata = features[4:]             # [4, H, W] - QP, Depth, PredMode, Boundary
